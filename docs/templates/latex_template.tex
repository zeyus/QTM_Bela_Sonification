...
$if(biblatex)$
\usepackage$if(biblio-style)$[backend=biber,style=$biblio-style$]$endif${biblatex}
...
$for(authors)$
\addauthor{{$authors.citation$ ($authors.student_id$, $authors.initials$)}}
$endfor$

\fancyhead[L]{\footnotesize{\sffamily \printall}.}
\fancyhead[C]{}
\fancyhead[R]{\footnotesize{\sffamily $course$.}}
\renewcommand{\footrulewidth}{0.25pt}

\fancyfoot[L]{\footnotesize{\sffamily $title$, ($year$).}}
...
\newgeometry{includemp, reversemp, left=1.0cm, marginparwidth=4.5cm, marginparsep=0.5cm}
  ...
  \marginpar{
    ...
    {\bfseries Programme}\\$degree$\\[1mm]
    {\bfseries Course}\\$course$\\[1mm]
    {\bfseries Supervisor}\\$supervisor$\\[1mm]
    {\bfseries Faculty}\\$faculty$\\
    $university$\\[2mm]

    {\bfseries Submitted:} $submitted$\\[2mm]

    {\bfseries Student Details}
    \begin{itemize}[align=parleft,left=1em..2em]
      \setlength\itemsep{0em}
      $for(authors)$
      \item $authors.name$\\ ID: $authors.student_id$\\ Initials: $authors.initials$
      $endfor$
    \end{itemize}
    ...
    \begin{itemize}[align=parleft,left=1em..2em]
      \setlength\itemsep{0em}
      \item \href{$repository$}{\color{NavyBlue}{Repository}} \ExternalLink
    \end{itemize}

    \vspace{2mm}

    {\bfseries License}\\
    Authors of papers retain copyright and release the work under a MIT Licence (\href{$licensepath$}{\color{NavyBlue}{MIT}}).
  }
\restoregeometry
$for(include-before)$
$include-before$
