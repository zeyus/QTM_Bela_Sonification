...
if (file.exists(trajectories_with_hilbert)) {
    trajectories_and_angles <- readRDS(trajectories_with_hilbert)
    ...
    trajectories <- trajectories %>%
      ...
      group_modify(~ {
        ...
        instantaneous_phase_angle <- pracma::angle(y_hilbert)
    ...
    trajectories$mean_instantaneous_phase_angle <- NA_real_

    # split the data into groups
    trajectories <- trajectories %>%
        group_by(experiment_id, condition, trial, mt_seq)
    timewise_pairs <- group_split(trajectories)
    trajectories <- trajectories %>% ungroup()

    # calculate the mean instantaneous phase angle for each time point
    num_groups <- nrow(
        trajectories %>%
        distinct(experiment_id, condition, trial, mt_seq))
    n_processed <- 0
    message("Calculating mean instantaneous phase angle...")
    flush.console()

    index_pairs_with_mean <- lapply(timewise_pairs, function(index_pair) {
        n_processed <<- n_processed + 1
        if (counter %% 1000 == 0) {
            message(paste0("  ", counter, " of ", num_groups))
            flush.console()
        }
        # get the mean instantaneous phase angle for each time point
        # for each subject pair
        mean_instantaneous_phase_angle <-
            mean(index_pair$instantaneous_phase_angle)

        # add the mean instantaneous phase angle to the data
        index_pair$mean_instantaneous_phase_angle <-
            mean_instantaneous_phase_angle
        index_pair
    })

    # join togethr using unlist
    trajectories_and_angles <- list2DF(
        lapply(
            setNames(
                seq_along(
                    index_pairs_with_mean[[1]]),
                names(index_pairs_with_mean[[1]])),
            \(i)
            unlist(lapply(index_pairs_with_mean, `[[`, i), FALSE, FALSE)))

    # save the trajectories with hilbert transform
    saveRDS(trajectories_and_angles, trajectories_with_hilbert)
    rm(trajectories_with_hilbert, trajectories, counter, index_pairs_with_mean,
       timewise_pairs, num_groups)
}
```
...
trajectories_angles_pairwise <- trajectories_and_angles %>%
    dplyr::select(-subj) %>%
    dplyr::distinct()

# allow for circular to convert radians to degrees
trajectories_angles_pairwise$mean_instantaneous_phase_angle <-
    circular::circular(
        trajectories_angles_pairwise$mean_instantaneous_phase_angle,
        type = "angles",
        units = "radians",
...
plt <- trajectories_angles_pairwise %>%
    ...
    ggplot(aes(
        x = timestamps,
        y = circular::deg(mean_instantaneous_phase_angle),
        color = trial)) +
    geom_line() +
    ...
    ggtitle(
        "Mean Instantaneous Phase Angles by Condition and Trial") +
    xlab("Time (s)") +
    ylab("Mean pairwise phase angle (degrees)")
...
trajectories_angles_pairwise %>%
    ...
    group_map(~ {
        plot(.x$mean_instantaneous_phase_angle)
    })
...
plot.circular(circular::deg(test_pair$mean_instantaneous_phase_angle), stack = TRUE, bins = 150, shrink = 1.5)
...
test_test <- trajectories_and_angles %>% dplyr::filter(experiment_id == "Subject Pair 1" & condition == "Sync" & trial == "Trial 1")

test_test$instantaneous_phase_angle_360 <- circular::circular(
    test_test$instantaneous_phase_angle,
    type = "angles",
    units = "radians",
    modulo = "pi")

test_test$instantaneous_phase_angle_180 <- circular::circular(
    test_test$instantaneous_phase_angle,
    type = "angles",
    units = "radians",
    modulo = "pi")
head(test_test)
cor.circular(
    test_test[subj == FO22J, ]$instantaneous_phase_angle_360,
    test_test$instantaneous_phase_angle_180,
    method = "spearman")
# 
