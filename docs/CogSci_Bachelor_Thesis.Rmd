...
```{r exp-graph, fig.align='center', fig.cap='Experimental design', out.width="100%"}
# @todo change font
graph <- "
digraph E {
    rankdir=\"LR\";
    
    node [shape=rect, group=main];

    Marker[label=\"Passive markers\"]
    MoCap[label=\"Cameras\"]
    QTM[label=\"QTM server\"]
    RT[label=\"Real-time API\", shape=ellipse]
    #Data[label=\"3D Data\", shape=none]
    #Events[label=\"Event Labels\", shape=none]
    Code[label=\"Sonification\", shape=ellipse]
    Experiment[label=\"Trial Runner\", shape=ellipse]
    Bela
    Audio[label=\"Audio output\"]

    edge [arrow=normal];

    {rank=same Marker -> MoCap -> QTM}
    QTM -> RT
    RT -> Bela [label=\"3D Data\"]
    ...
    Bela:n -> Experiment
    Experiment -> RT [label=\"Event Labels\"]
...
gg.gauge <- function(pos,breaks=c(0,180)) {
  ...
  pos$angle_sd_pct <- pos$angle_sd*(limit) / 100
...
```{r instantaneous-phase-angle-circular-plot, fig.cap="Plot of mean pairwise phase angles for each subject pair", fig.align="center", out.width="100%"}
...
```{r mean-instantaneous-phase-angle-circular-plot, fig.cap="Plot of mean pairwise phase angles by condition", fig.align="center", out.width="100%"}
...
trajectories_angles_pairwise_for_model <- trajectories_angles_pairwise %>%
    ...
    dplyr::summarise(
        ...
        instantaneous_phase_angle_diff = mean(instantaneous_phase_angle_diff),
        .groups = "drop")
# first we need to relevel the condition factor so that the first level is the "No Sonification" condition
trajectories_angles_pairwise_for_model$condition <- factor(trajectories_angles_pairwise_for_model$condition, levels = c("No Sonification", "Task", "Sync"))
# starting with distance
m_dist_null <- lme4::lmer(
  delta_y ~ 1 + (1|experiment_id),
  data = trajectories_angles_pairwise_for_model,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)))

m_dist <- lme4::lmer(
  delta_y ~ condition + (1|experiment_id),
  data = trajectories_angles_pairwise_for_model,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)))

# now for phase angle
m_angle_null <- lme4::lmer(
  instantaneous_phase_angle_diff ~ 1 + (1|experiment_id),
  data = trajectories_angles_pairwise_for_model,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)))
m_angle <- lme4::lmer(
  instantaneous_phase_angle_diff ~ condition + (1|experiment_id),
  data = trajectories_angles_pairwise_for_model,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)))
...
This has "add_glance_table"

Comparing the distance model with the null distance model

```{r regression-results-dist}

# distance
tbl_dist_null <- tbl_regression(
  m_dist_null,
  labels = list (condition ~ "Condition"),
  intercept = TRUE,
  tidy_fun = broom.mixed::tidy) %>%
  add_glance_table()
tbl_dist <- tbl_regression(
  m_dist,
  labels = list (condition ~ "Condition"),
  intercept = TRUE,
  tidy_fun = broom.mixed::tidy) %>%
  add_glance_table()
tbl_merge(tbls = list(tbl_dist_null, tbl_dist),
tab_spanner = c("Null (Mean)", "∆ Distance ~ Condition")) %>%
  modify_table_body(~.x %>% arrange(row_type == "glance_statistic")) %>%
  as_kable_extra(
    caption = "Comparison null distance model to distance model",
    booktabs = TRUE) %>% kable_styling(font_size=7)

```

This has "Exponentiate = TRUE", and no "add_global_p"

This compares the angle model with the null angle model

```{r regression-results-angle}
...
tbl_angle_null <- tbl_regression(
  ...
  tidy_fun = broom.mixed::tidy) %>%
  add_glance_table()
tbl_angle <- tbl_regression(m_angle,
  labels = list (condition ~ "Condition"),
  intercept = TRUE,
  tidy_fun = broom.mixed::tidy) %>%
  add_glance_table()
tbl_merge(
  tbls = list(tbl_angle_null, tbl_angle),
  tab_spanner = c("Null (Mean)", "∆ Angle° ~ Condition")) %>%
  modify_table_body(~.x %>% arrange(row_type == "glance_statistic")) %>%
  as_kable_extra(
    caption = "Comparison null angle model to angle model",
    booktabs = TRUE) %>% kable_styling(font_size=7)
...
this is using the formula `delta_y ~ condition`
...
tbl_regression(lm_dist) %>%
  as_kable_extra(
    caption = "Linear model for distance",
    booktabs = TRUE) %>% kable_styling(font_size=7)

```
this is using the formula `angle ~ condition`
```{r lm-ang}

lm_dist <- lm(
  instantaneous_phase_angle_diff ~ condition,
  data = trajectories_angles_pairwise_for_model)

tbl_regression(lm_dist) %>%
  as_kable_extra(
    caption = "Linear model for angle",
    booktabs = TRUE) %>% kable_styling(font_size=7)
