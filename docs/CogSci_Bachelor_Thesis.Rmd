...
angular_trajectories$condition <- factor(angular_trajectories$condition, levels = c("No Sonification", "Task", "Sync"), ordered=TRUE)
...
Other benefits of auditory perception that speak for sonification are parallel listening (the ability to monitor and process multiple audio sources), affective response (increased learning and engagement), and finally rapid detection [@kramerSonificationReportStatus1999 – humans are able to react faster to sound than to any other type of stimulus, achieving reaction times of around 160 ms in simple reaction time experiments [@kosinskiLiteratureReviewReaction2008]. 
...
ggplot(
    ...
    labs(
        x = "Condition",
        y = "Pairwise position delta")

# library(lme4)

# # Fit a mixed-effects model to the data
# fit <- lmer(delta_y ~ condition + (1|experiment_id) + (1|trial), data = trajectories_angles_pairwise)

# # Extract the fixed effects coefficients
# fixed_effects <- fixef(fit)

# # Calculate the mean distance delta for each condition
# mean_delta_by_condition <- cbind(
#   condition = levels(trajectories_angles_pairwise$condition),
#   delta_y = c(
#     fixed_effects[1],
#     fixed_effects[2:3] + fixed_effects[1]))

# # Extract the residuals
# residuals <- resid(fit)

# # Calculate the standard deviation for each condition
# sd_delta_by_condition <- tapply(residuals, trajectories_angles_pairwise$condition, sd)
...
Instantaneous phase angles provide a way to determine differences in position, acceleration…
...
gg.gauge <- function(pos,breaks=c(0,180)) {
  ...
  get.poly <- function(a,b,r1=0.0,r2=1.0,cond=NA) {
    ...
    cond <- factor(cond,levels=levels(pos$condition), ordered=TRUE)
    return(data.frame(
        x=x,
        y=y,
        condition=cond,
...
plot_title <- paste(
    "Distribution density plot of normalized dynamic time warp path distance between paired trajectories by condition.",
    "The mean normalized distance is shown as a point (+), the 20th and 80th percentiles are shown as dashed lines",
    "and the 50th percentile is shown as a solid line.",
    "No Sonification condition normalized distance mean  = ",
    dtw_res$dist_mean[dtw_res$condition == "No Sonification"],
    "(sd = ",
    dtw_res$dist_sd[dtw_res$condition == "No Sonification"],
    ") degrees, task sonification condition normalized distance mean = ",
    dtw_res$dist_mean[dtw_res$condition == "Task"],
    "(sd = ",
    dtw_res$dist_sd[dtw_res$condition == "Task"],
    ") degrees, sync sonification condition normalized distance mean = ",
...
was `r ipa_res$angle_mean[ipa_res$condition == "No Sonification"]`° ± `r ipa_res$angle_sd[ipa_res$condition == "No Sonification"]`° (SD) for the no sonification condition,
...
```{r analysis-aov, include=FALSE, eval=FALSE}
...
anova_angle <- Directional::lr.circaov(
  trajectories_angles_pairwise_mean_trial$angle_mean,
  trajectories_angles_pairwise_mean_trial$condition,
  rads = FALSE)
