...
plt1 = gg.gauge(trajectories_angles_pairwise_mean_trial) +
...
trajectories_angles_pairwise_mean_trial <- trajectories_angles_pairwise %>%
    dplyr::group_by(condition) %>%
    dplyr::summarise(
        angle_sd = as.numeric(circular::sd(instantaneous_phase_angle_diff_c)),
        angle_mean = as.numeric(mean.circular(instantaneous_phase_angle_diff_c)),
        angle_sem = as.numeric(circular::sd(instantaneous_phase_angle_diff_c)/sqrt(n())),
        angle_var = as.numeric(circular::angular.variance(instantaneous_phase_angle_diff_c)),
        .groups = "drop")
plt2 = gg.gauge(trajectories_angles_pairwise_mean_trial) +
      theme(legend.position = "bottom")+
      labs(title = "Mean Phase Angle of Trajectories",
           subtitle = "Mean of all timepoints for each condition")
plt2

plt3 = grid_arrange_shared_legend(list(plt1, plt2), nrow = 1, ncol = 2)
ggsave("figures/mean_condition_phase_angles_mean_sd.png", plt3, width = 12, height = 8)
