...
# distance
tbl_dist_null <- tbl_regression(m_dist_null, intercept = TRUE) %>%
  add_global_p() %>% 
  add_glance_table()
tbl_dist <- tbl_regression(m_dist, intercept = TRUE) %>%
  add_global_p() %>% 
  add_glance_table()
tbl_merge(tbls = list(tbl_dist_null, tbl_dist)) %>%
  modify_table_body(~.x %>% arrange(row_type == "glance_statistic")) %>%
  as_kable_extra(format = "latex") %>% kable_styling(font_size=7)
...
# angle
tbl_angle_null <- tbl_regression(m_angle_null, intercept = TRUE, exponentiate = TRUE)
tbl_angle <- tbl_regression(m_angle, intercept = TRUE, exponentiate = TRUE)
tbl_merge(tbls = list(tbl_angle_null, tbl_angle)) %>%
  as_kable_extra(format = "latex") %>% kable_styling(font_size=7)
...
```{r anova-dist}
all_models <- list(m_dist_null, m_dist, m_angle_null, m_angle)
broom_outputs <- purrr::map(all_models, broom::tidy)
...
df %>% kbl(format = "latex") %>% kable_styling(font_size=7)
```
...
lm_dist <- lm(delta_y ~ condition, data = trajectories_angles_pairwise_for_model)
