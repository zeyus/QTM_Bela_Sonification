...
```{r pairwise-phase-angles, echo=FALSE, fig.cap="Plot of pairwise phase angles for each subject pair", fig.align="center", out.width="100%"}
...
gg.gauge <- function(pos,breaks=c(0,180)) {
  ...
  plt = ggplot() + geom_polygon(data=get.poly(rel_breaks[1],rel_breaks[2]),aes(x,y),color="#00000033", fill="#00000033")
  ...
  for (i in 1:nrow(pos)) {
    ...
    plt = plt + 
      geom_polygon(
        data=d,
        aes(x,y, color=stage(condition, after_scale = alpha(color, 0.5)),
          fill=stage(condition, after_scale = alpha(fill, 0.5))))
  }
  plt = plt +
    geom_label_repel(
        data=d_labels,
...
grid_arrange_shared_legend(plots, nrow = 3, ncol = 2)
