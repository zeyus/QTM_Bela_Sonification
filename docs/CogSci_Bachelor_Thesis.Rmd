...
bela_buffer_size <- 32
bela_n_channels <- 2
undertone_stimulus <- "../src/res/simple_As3.wav"
overtone_stimulus <- "../src/res/simple_f4.wav"
...
summary(dtw_results)
# let's plot the mean and standard deviation of the normalized distance
# for each condition

# get the mean and standard deviation
dtw_results_mean <- dtw_results %>%
  group_by(condition) %>%
  summarise(
    mean_distance = mean(normalizedDistance),
    sd_distance = stats::sd(normalizedDistance)
  )

# set condition order to "No Sonification", "Task", "Sync"
 

# plot the mean and standard deviation
ugd(width=figure_width, width=figure_height)

ggplot(dtw_results_mean, aes(x=condition, y=mean_distance, fill=condition)) +
  geom_bar(stat="identity") +
  geom_errorbar(aes(ymin=mean_distance-sd_distance, ymax=mean_distance+sd_distance), width=0.2, color = "white") +
  theme_hc(style = "darkunica") +
  labs(
    x = "Condition",
    y = "DTW Normalized distance"
  ) +
  theme(
...
ugd_save("figures/dtw_summary.png", width=figure_width, width=figure_height, zoom = 3)
dev.off()
...
ugd_save("figures/parwise_position_delta.png", width=figure_width, width=figure_height, zoom = 3)
dev.off()

```

```{r pairwise-positiondelta, fig.cap="Plot of pairwise position delta for each subject pair", fig.align="center", out.width="100%"}
...
trajectories_angles_pairwise_mean_condition <- trajectories_angles_pairwise_mean_trial %>%
...
trajectories_angles_pairwise_mean_condition$angle_sd <- sqrt(trajectories_angles_pairwise_mean_condition$angle_var)

# remove the angle_var column
trajectories_angles_pairwise_mean_condition <- trajectories_angles_pairwise_mean_condition %>%
    dplyr::select(-angle_var)

ugd(width=figure_width, width=figure_height)
gg.gauge(trajectories_angles_pairwise_mean_condition, breaks = c(0, 90)) +
      theme(legend.position = "bottom")
ugd_save("figures/mean_condition_phase_angles_mean_sd.png", width=figure_width, width=figure_height, zoom = 3)
dev.off()
...
plot_title <- paste(
    ...
    round(
        trajectories_angles_pairwise_mean_condition$angle_mean[
            trajectories_angles_pairwise_mean_condition$condition == "No Sonification"
        ], 
        2),
    "(sd = ",
    round(
        trajectories_angles_pairwise_mean_condition$angle_sd[
            trajectories_angles_pairwise_mean_condition$condition == "No Sonification"
        ],
        2),
    ") degrees, sync sonification condition mean phase angle = ",
        round(
        trajectories_angles_pairwise_mean_condition$angle_mean[
            trajectories_angles_pairwise_mean_condition$condition == "Sync"
        ], 
        2),
    "(sd = ",
        round(
    ...
    ") degrees, task sonification condition mean phase angle = ",
        round(
        trajectories_angles_pairwise_mean_condition$angle_mean[
            trajectories_angles_pairwise_mean_condition$condition == "Task"
        ], 
        2),
    "(sd = ",
        round(
...
ugd_save("figures/spec_tones.png", width=figure_width, width=figure_height, zoom = 3)
dev.off()
